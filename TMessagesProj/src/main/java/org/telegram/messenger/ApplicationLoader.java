    ...
    public void addItemOptions(ItemOptions itemOptions) {
        if (SMSJobController.getInstance(UserConfig.selectedAccount).isAvailable()) {
            ...
            if (MessagesController.getGlobalMainSettings().getBoolean("newppsms", true)) {
                text = PremiumPreviewFragment.applyNewSpan(text.toString());
            }
            boolean withError = AndroidUtilities.isInAirplaneMode(LaunchActivity.instance) || SMSJobController.getInstance(UserConfig.selectedAccount).hasError();
